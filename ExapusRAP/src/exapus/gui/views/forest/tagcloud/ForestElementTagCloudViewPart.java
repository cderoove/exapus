...
public class ForestElementTagCloudViewPart extends SelectedForestElementBrowserViewPart {
	...
	protected void registerHtml(String id, String html) {
		RWT.getUISession().setAttribute(id, html);
	}

	protected String createImageUrl(String id) {
		StringBuilder url = new StringBuilder();
		url.append( RWT.getServiceManager().getServiceHandlerUrl(HANDLER));
		url.append("&htmlId=").append(id);
		url.append("&nocache=").append(System.currentTimeMillis());
		return url.toString();	
	}

	private static class SubTagsHtmlHandler implements ServiceHandler {

		@Override
		public void service(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
			String id = request.getParameter("htmlId");
			if(id != null) {
				String html = (String) RWT.getUISession().getAttribute(id);
				if (html != null) {
